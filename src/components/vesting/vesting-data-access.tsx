...
import { getVestingProgram, getVestingProgramId, Vesting } from '@project/anchor'
...
export function useVestingProgramAccount({ account }: { account: PublicKey }) {
  ...
  const createEmployeeVesting = useMutation<string, Error, CreateEmployeeArgs>({
    ...
    mutationFn: ({ startTime, endTime, totalAmount, cliffTime, beneficiary }) =>
      program.methods.createEmployeeAccount(new BN(startTime), new BN(endTime), new BN(totalAmount), new BN(cliffTime)).accounts({ beneficiary: new PublicKey(beneficiary), vestingAccount: account }).rpc(),
    onSuccess: (signature) => {
      transactionToast(signature)
      return accounts.refetch()
    },
